...
# Update the request tracking data structure for the current slot
    ...
    def update_action_space(self, satellite_list, non_neighbor_uavs, slot, federated_update): #for MAB and all
        #print('update_action_space')
        # Clear the existing action space
        self.action_space = []
    ...
    def run(self, current_time, communication, communication_schedule, slot, satellites, no_of_content_each_category, uav_content_generation_period, epsilon, uavs):
        ...
        if (slot - 1) % uav_content_generation_period == 0:
            ...
            self.cache_content(communication, communication_schedule, slot, uavs, satellites) #while the loop is in 11th we are giving the slot no 10 becase we are eager to cache the content those would be out of range after 10th slot
